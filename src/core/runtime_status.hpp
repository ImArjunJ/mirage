...
[[nodiscard]] bool write_runtime_status_json(
    const std::filesystem::path& status_path, int pid, const config& cfg,
...
public:
    ...
    void client_stream_updated(uint64_t client_id,
                               receiver_client_stream_status stream) override;
    void client_media_updated(uint64_t client_id,
                              receiver_client_media_status media) override;

    [[nodiscard]] bool write();

private:
    [[nodiscard]] bool has_client(protocol protocol_id) const;
