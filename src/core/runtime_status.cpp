...
bool write_runtime_status_json(const std::filesystem::path& status_path, int pid,
...
runtime_status_tracker::runtime_status_tracker(
    std::filesystem::path status_path, int pid, const config& cfg, std::string ip,
    std::string iface_name, std::filesystem::path identity_path, int64_t started,
...
void runtime_status_tracker::client_stream_updated(uint64_t client_id,
    ...
    auto existing = std::ranges::find(client->streams, stream.kind,
                                      &receiver_client_stream_status::kind);
...
bool runtime_status_tracker::has_client(protocol protocol_id) const {
