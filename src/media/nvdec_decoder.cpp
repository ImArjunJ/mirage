...
result<nvdec_decoder> nvdec_decoder::create(video_codec codec) {
#if MIRAGE_HAS_NVDEC
    AVCodecID codec_id;
    switch (codec) {
        ...
        case video_codec::av1:
            ...
            break;
        default:
            return std::unexpected(mirage_error::decode("NVDEC: unsupported codec"));
    }
    auto impl_ptr = std::make_unique<impl>();
    impl_ptr->codec = avcodec_find_decoder(codec_id);
    if (!impl_ptr->codec) {
        return std::unexpected(mirage_error::decode("NVDEC: codec not found"));
    }
    ...
    if (avcodec_open2(impl_ptr->ctx, impl_ptr->codec, nullptr) < 0) {
        return std::unexpected(mirage_error::decode("NVDEC: failed to open codec"));
    }
    ...
    if (!impl_ptr->packet || !impl_ptr->hw_frame || !impl_ptr->sw_frame) {
        return std::unexpected(mirage_error::decode("NVDEC: failed to allocate frames"));
    }
    return nvdec_decoder{std::move(impl_ptr), codec};
#else
    (void)codec;
    return std::unexpected(mirage_error::decode("NVDEC: CUDA not available on this system"));
#endif
}
result<std::optional<decoded_frame>> nvdec_decoder::decode(std::span<const std::byte> nal) {
#if MIRAGE_HAS_NVDEC
    ...
    if (ret < 0 && ret != AVERROR(EAGAIN)) {
        return std::unexpected(mirage_error::decode("NVDEC: send packet failed"));
    }
    ...
    if (ret < 0) {
        return std::unexpected(mirage_error::decode("NVDEC: receive frame failed"));
    }
    ret = av_hwframe_transfer_data(impl_->sw_frame, impl_->hw_frame, 0);
    if (ret < 0) {
        av_frame_unref(impl_->hw_frame);
        return std::unexpected(mirage_error::decode("NVDEC: GPU to CPU transfer failed"));
    }
...
result<std::vector<decoded_frame>> nvdec_decoder::flush() {
#if MIRAGE_HAS_NVDEC
...
nvdec_decoder::nvdec_decoder(std::unique_ptr<impl> impl_ptr, video_codec codec)
    : impl_(std::move(impl_ptr)), codec_(codec) {}
nvdec_decoder::nvdec_decoder(nvdec_decoder&&) noexcept = default;
nvdec_decoder& nvdec_decoder::operator=(nvdec_decoder&&) noexcept = default;
nvdec_decoder::~nvdec_decoder() = default;
}  // namespace mirage::media
